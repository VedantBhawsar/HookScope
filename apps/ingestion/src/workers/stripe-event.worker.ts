...
export function createStripeWorker(connection: ConnectionOptions, deps: WorkerDeps) {
  const worker = new Worker<StripeEventJob>(
    ...
    async (job) => {
      ...
      // ── Step 4: Get endpoint for custom headers and forward to destination
      const endpoint = await deps.prisma.endpoint.findUnique({
        where: { id: endpointId },
        select: { customHeaders: true },
      })
...
async function forwardAndPersist(
  ...
  try {
    ...
    headers["content-type"] = "application/json" // Always enforce JSON
